...
{
    public class BasicSpecInvoke : BasicInfoInvoke
    {
        /// <summary>
        ...
        public override bool GetNCDataJoinRFID(out DataTable dt, out string msg)
        {
            ...
            try
            {
                ...
                if (dt != null)
                {
                    ...
                    if (dt.Columns.Contains("SpecName"))
                    {
                        using (Gold.DAL.GoldEntities context = new DAL.GoldEntities())
                        {
                            ...
                            {
                                string whcode = dr["SpecName"].ToString();
        ...
        public override bool SaveToRFID(System.Data.DataTable dt, out string msg)
        {
            ...
            {
                string convertMsg = "";
                List<Specifications> list = GetModelFromDataTable(dt, out convertMsg);
                if (list.Count == 0)
                {
                ...
                else
                {
                    ...
                    {
                        //查询系统中所有仓库编码
                        List<string> RFIDKeyList = (from r in context.Specifications select r.SpecName).ToList<string>();
                        //NC数据集中与现有系统已重复的数据主键
                        List<int> delKeyList = new List<int>();
                        for (int i = 0; i < list.Count; i++)
                        {
                            if (RFIDKeyList.Contains(list[i].SpecName))
                        ...
                        //变量过滤后的实体集，将其加入数据库
                        foreach (Specifications newModel in list)
                        {
                            context.Specifications.AddObject(newModel);
                        }
            ...
            {
                LogHelper.WriteLog(Utility.LogHelper.LogLevel.Error, "Specifications SaveToRFID方法出错", ex);
                return false;
            }
        }
        ...
        /// <param name="msg"></param>
        /// <returns></returns>
        private List<DAL.Specifications> GetModelFromDataTable(DataTable dt, out string msg)
        {
            List<DAL.Specifications> list = new List<DAL.Specifications>();
            msg = "";

            //接口协议文档中定义的字段
            Dictionary<string, string> dataFieldNameDic = new Dictionary<string, string>();
            dataFieldNameDic.Add("SpecCode", "规格编码");
            dataFieldNameDic.Add("SpecName", "规格名称");

            if (dt.Rows.Count == 0)
            {
                msg = "用友系统返回数据集中无数据！";
                return new List<Specifications>();
            }
            ...
            foreach (DataRow dr in dt.Rows)
            {
                Specifications newModel = new Specifications();

                newModel.Comment = DataCheckHelper.GetCellString(dr["SpecCode"]);
                newModel.SpecName = DataCheckHelper.GetCellString(dr["SpecName"]);


                List<Specifications> existWareHouse = (from r in list where r.SpecName == newModel.SpecName select r).ToList<Specifications>();
