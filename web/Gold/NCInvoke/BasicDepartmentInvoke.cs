...
{
    public class BasicDepartmentInvoke : BasicInfoInvoke
    {
        /// <summary>
        ...
        public override bool SaveToRFID(System.Data.DataTable dt, out string msg)
        {
            ...
            {
                string convertMsg = "";
                List<Department> list = GetModelFromDataTable(dt, out convertMsg);
                if (list.Count == 0)
                {
                ...
                else
                {
                    ...
                    {
                        //查询系统中所有仓库编码
                        List<string> RFIDKeyList = (from r in context.Department select r.DepartmentName).ToList<string>();
                        //NC数据集中与现有系统已重复的数据主键
                        List<int> delKeyList = new List<int>();
                        for (int i = 0; i < list.Count; i++)
                        {
                            if (RFIDKeyList.Contains(list[i].DepartmentName))
                        ...
                        if (list.Count == 0)
                        {
                            msg = "保存成功!影响行数：0。用友系统中部门数据已全部存在于RFID系统中。";
                            return true;
                        }

                        //变量过滤后的实体集，将其加入数据库
                        foreach (Department newModel in list)
                        {
                            context.Department.AddObject(newModel);
                        }
            ...
            {
                LogHelper.WriteLog(Utility.LogHelper.LogLevel.Error, "Department SaveToRFID方法出错", ex);
                return false;
            }
        }
