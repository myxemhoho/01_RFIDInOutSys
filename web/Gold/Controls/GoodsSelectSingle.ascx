<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="GoodsSelectSingle.ascx.cs"
    Inherits="Gold.Controls.GoodsSelectSingle" %>
...
//        }
    ...
    //gridViewClientID是GridView的客户端ID
    //theFireRadioButton 是触发选择的那个CheckBox对象，调用时使用This
    function RadioSelectOne( theFireRadioButton) {
        

        //方法2 在IE，Chrome，FireFox均通用
        var grid = document.getElementById('<%=gvGoods.ClientID %>'); //(gridViewClientID); //获取
        //var theChkCheckAll = allCheckBoxObj; //触发全选的CheckBox
        
        var elements = grid.getElementsByTagName("input");
        var j = 0;
        for (j = 0; j < elements.length; j++) {
            if (elements[j] != null && elements[j].type == "radio") {
//                var checkedValue = theChkCheckAll.checked;
                //                elements[j].checked = checkedValue;
                if (elements[j] != theFireRadioButton)
    ...
    <div style="padding: 0px 5px 0px 5px;">
        ...
        <div style="width: 99%; height: 405px;" class="divScroll" id="gridviewContainer"
            ...
            <asp:GridView runat="server" ID="gvGoods" AllowPaging="false" ShowHeader="false"
                PageSize="20" OnPageIndexChanging="gvGoods_PageIndexChanging" Width="1340px"
                ...
                <Columns>
                    <asp:TemplateField ItemStyle-Width="50px" ItemStyle-HorizontalAlign="Center" HeaderText="*">
                        <ItemTemplate>
                            ...
                            <asp:RadioButton ID="rbSelect" runat="server" GroupName="RadioButtonGroup" onclick="javascript:RadioSelectOne(this);" />
                        </ItemTemplate>
                        <HeaderTemplate>
                            <%--全选<input id="CheckAll" type="checkbox" onclick="selectAll1(this);" />--%>
                        </HeaderTemplate>
                    </asp:TemplateField>
                    <asp:BoundField DataField="CargoCode" HeaderText="商品编码" HtmlEncode="false" ItemStyle-Width="200px"
                        SortExpression="CargoCode" />
                    <asp:BoundField DataField="CargoName" HeaderText="名称" HtmlEncode="false" ItemStyle-Width="500px"
                    ...
                    <asp:BoundField DataField="Comment" HeaderText="备注" HtmlEncode="false" ItemStyle-Width="400px"
