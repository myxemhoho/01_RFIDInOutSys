    ...
    public class FileUpload : INotifyPropertyChanged
    {
        ...
        public string ParsingResult
        {
            ...
            set
            {
                ...
                {
                    if (PropertyChanged != null)
                        PropertyChanged(this, new PropertyChangedEventArgs("ParsingResult"));
                    if (StatusChanged != null)
                        StatusChanged(this, null);
                });
            }
        ...
        public FileUpload(Dispatcher dispatcher, Uri uploadUrl, FileInfo fileToUpload, string fileType)
            : this(dispatcher, uploadUrl)
        {
            File = fileToUpload;
            FileType = fileType;
        ...
        private void WriteCallback(IAsyncResult asynchronousResult)
        {
            ...
            if (resizeStream != null )
                fileStream = (Stream)resizeStream;
            else
            {
                try
                {
        ...
        private void ParsingFileOnServer()
        {
            UriBuilder ub = new UriBuilder(UploadUrl);
            ub.Query = string.Format("{1}filename={0}&ParsingFile=true&FileType={2}&Random={3}", 
                File.Name, 
                string.IsNullOrEmpty(ub.Query) ? "" : ub.Query.Remove(0, 1) + "&",
                FileType,
                new Random().Next());
